...
export function LeadEngineStory() {
  return (
    <>
      <section className="border-b border-border bg-slate-950 py-24 text-white lg:py-32">
        <div className="mx-auto max-w-6xl px-6">
          <div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:items-end">
            ...
            <p className="max-w-xl text-lg leading-8 text-slate-300">Your customer does not care what platform we use. They care whether the phone rings with people who need the work you do. That is the system we build, launch, and improve with you.</p>
          </div>
          <div className="mt-14 grid gap-px overflow-hidden rounded-3xl border border-slate-800 bg-slate-800 sm:grid-cols-2">{SYSTEM.map(([number, title, body]) => <div key={number} className="bg-slate-950 p-7 sm:p-9"><p className="text-sm font-bold text-primary">{number}</p><h3 className="mt-12 text-xl font-semibold">{title}</h3><p className="mt-3 text-sm leading-6 text-slate-400">{body}</p></div>)}</div>
        </div>
      </section>
