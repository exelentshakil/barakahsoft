  ...
  return (
    <section id="team" className="scroll-mt-24 border-t border-[#d9e8f4] bg-[#f8fbfe] py-20 lg:py-28">
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-2">{people.map((person) => <article key={person.name} className="group overflow-hidden rounded-2xl border border-[#c8ddec] bg-white shadow-[0_8px_24px_rgba(7,40,77,0.05)] transition hover:-translate-y-1 hover:shadow-[0_18px_40px_rgba(7,40,77,0.12)]"><div className="grid min-h-[270px] sm:grid-cols-[190px_1fr]">{person.photoUrl ? <div className="relative h-64 overflow-hidden bg-[#eef7ff] sm:h-auto"><Image src={person.photoUrl} alt={person.name} width={190} height={270} className="absolute inset-0 h-full w-full object-cover object-top grayscale-[15%] transition duration-500 group-hover:scale-105" /></div> : <div className="flex h-24 items-center justify-center bg-[#eef7ff] text-3xl font-bold text-[#0c68c8] sm:h-auto">{person.name.split(" ").map((w) => w[0]).slice(0, 2).join("")}</div>}<div className="flex flex-col p-7"><p className="text-xl font-bold text-[#07284d]">{person.name}</p><p className="mt-1 whitespace-nowrap text-xs font-semibold text-[#0c68c8]">{person.role}</p><p className="mt-6 text-sm leading-6 text-[#60778d]">{person.bio}</p><div className="mt-auto pt-7"><div className="h-1 w-10 rounded-full bg-[#ffd12d]" /></div></div></div></article>)}</div>
