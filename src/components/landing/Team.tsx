    ...
    <section id="team" className="border-t border-border py-24">
      <div className="mx-auto max-w-4xl px-6">
        <div className="text-center">
          <SectionEyebrow icon={Users}>Who you're working with</SectionEyebrow>
          <h2 className="font-display text-3xl font-bold tracking-tight sm:text-4xl">Real people, not a ticket queue</h2>
        </div>
          ...
          {TEAM.map((member, i) => (
            <RevealItem key={member.name} index={i}>
              <div className="flex h-full flex-col gap-4 rounded-2xl border border-border bg-card p-6 shadow-card">
                ...
                <p className="text-sm text-muted-foreground">&ldquo;{member.quote}&rdquo;</p>
              </div>
            </RevealItem>
          ))}
        </RevealGroup>
