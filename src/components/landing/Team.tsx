...
export function Team() {
  return (
    <section id="team" className="border-t border-border bg-[#f8f7fc] py-20 text-foreground lg:py-24">
      <div className="mx-auto max-w-5xl px-6">
        <div className="text-center">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-muted-foreground">The system is automated where it should be. The judgment is not. We research the market, shape the offer, edit the creative, watch the campaign, and improve what the data tells us.</p>
        </div>
        <RevealGroup className="mt-14 grid gap-6 sm:grid-cols-2">
          {TEAM.map((member, i) => (
            <RevealItem key={member.name} index={i}>
              <div className="flex h-full flex-col gap-4 rounded-2xl border border-border bg-card p-6 shadow-card">
                ...
                <p className="text-sm leading-6 text-muted-foreground">&ldquo;{member.quote}&rdquo;</p>
              </div>
            </RevealItem>
          ))}
        </RevealGroup>
