...
export function PremiumLeadHomepage({ payload }: { payload: SitePayload }) {
  ...
  const stats = [
    payload.proof.rating ? [`${payload.proof.rating}★`, "average rating"] : null,
    payload.proof.reviewCount ? [`${payload.proof.reviewCount}+`, "real reviews"] : null,
    payload.services.length ? [`${payload.services.length}`, "services listed"] : null,
    featuredAreas.length ? [`${featuredAreas.length}`, "service areas"] : null,
  ...
  return (
    <div style={style} className="bg-background pb-20 text-foreground lg:pb-0">
      ...
      <main>
        ...
        {stats.length > 0 && <section className="border-b border-border bg-muted/40 py-8"><div className="mx-auto grid max-w-7xl gap-4 px-6 sm:grid-cols-2 lg:grid-cols-4">{stats.map(([value, label]) => <div key={label} className="rounded-2xl border border-border bg-card px-5 py-4"><p className="font-display text-3xl font-bold tracking-tight text-primary">{value}</p><p className="mt-1 text-xs font-bold uppercase tracking-[0.14em] text-muted-foreground">{label}</p></div>)}</div></section>}
        ...
        {payload.faq.length > 0 && <section id="faq" className="border-b border-border py-20 lg:py-28"><div className="mx-auto max-w-3xl px-6"><p className="text-center text-xs font-bold uppercase tracking-[0.2em] text-primary">Useful answers</p><h2 className="mt-3 text-center font-display text-4xl font-bold tracking-tight">Questions customers ask before they call.</h2><div className="mt-10 divide-y divide-border rounded-2xl border border-border bg-card">{payload.faq.slice(0, 8).map((item) => <details key={item.slug} className="group p-5"><summary className="cursor-pointer list-none pr-8 text-sm font-bold marker:hidden">{item.h2}<span className="float-right text-primary transition group-open:rotate-45">+</span></summary><p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">{item.body_content}</p></details>)}</div></div></section>}
