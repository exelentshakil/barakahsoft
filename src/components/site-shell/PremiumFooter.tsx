...
// into an image, with tel:/mailto: links.
    ...
    <footer className="border-t border-border bg-muted/40">
      <div className="mx-auto max-w-6xl px-6 py-14">
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-5">
            ...
            {(payload.socialUrls.length > 0) && (
              <div className="mt-4 flex gap-3">
                {payload.socialUrls.slice(0, 4).map((url) => (
                  <a key={url} href={url} target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-foreground">
          ...
          <div>
            ...
            <ul className="mt-3 space-y-2 text-sm text-muted-foreground">
              {payload.nap.phone && (
                <li>
                  <a href={`tel:${payload.nap.phone}`} className="hover:text-foreground">
                    {payload.nap.phone}
                  </a>
                </li>
              )}
              {payload.nap.email && (
                <li>
                  <a href={`mailto:${payload.nap.email}`} className="hover:text-foreground">
                    {payload.nap.email}
                  </a>
                </li>
              )}
              {payload.nap.address && <li>{payload.nap.address}</li>}
            </ul>
          </div>
        </div>
        ...
        <p className="mt-6 max-w-3xl text-xs text-muted-foreground">
          ...
          availability, and service area are subject to confirmation at time of booking. This site was built with BarakahSoft.
        </p>
      </div>
