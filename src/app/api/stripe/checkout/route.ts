...
import { getStripe, BUILD_PRICE_ID, HOSTING_PRICE_ID, HOSTING_SUPPORT_PRICE_ID } from "@/lib/stripe";
...
export async function POST(req: Request) {
  ...
  if (tier !== "hosting" && tier !== "hosting_support") {
    return NextResponse.json({ error: "tier must be 'hosting' or 'hosting_support'" }, { status: 400 });
  }
  ...
  const hostingPriceId = tier === "hosting_support" ? HOSTING_SUPPORT_PRICE_ID : HOSTING_PRICE_ID;
  if (!BUILD_PRICE_ID || !hostingPriceId) {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    line_items: [
      ...
      { price: BUILD_PRICE_ID, quantity: 1 },
    ],
