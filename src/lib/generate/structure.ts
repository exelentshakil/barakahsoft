...
- If this batch contains reviews, make it unmistakably testimonial content: build one accessible slider whose section has data-review-slider, a track container with data-review-track, a header with section title and sleek circular arrow icon buttons:
  ...
  Each slide is an equal-height card (display: flex; flex-direction: column; justify-content: space-between) showing the supplied rating as ★★★★★ in gold (styled with var(--bs-primary-on-surface)), followed by a semantic blockquote containing only the verbatim quote in a <p>, and an author block in a <cite> with a verified customer badge. JavaScript-off fallback remains horizontally scrollable.
...
5. HERO EXCELLENCE: The Hero section is the highest-value part of the page. It MUST use a massive, premium layout (e.g. min-height: 90vh). The background MUST be a full-width image with a dark overlay, NEVER a split white/image layout. The text, trust badges, and forms MUST sit ON TOP of this dark image overlay. The main call-to-action button MUST use the exact classes 'site-cta site-cta--primary'.
