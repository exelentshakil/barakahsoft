...
export async function authorBody(input: AuthorInput, systemCss: string, chromeCss: string): Promise<Body | null> {
  const prompt = [
    ...
    `Sections must not all be the same shape. Vary composition and vary ground.`,
    ``,
    `LAND INSIDE THE BAND, and note that each of these has a ceiling as well as a floor. A page that pushes every dimension to its limit at once does not read as bold, it reads as broken — a stack of enormous headings with almost nothing under them.`,
    `- The hero is 70-92vh with a 4-12 word headline, AND carries supporting copy and a call to action.`,
    `- var(--fs-8) is the hero headline and nothing else. Section headings are var(--fs-5) or var(--fs-6). A card heading is var(--fs-3) or var(--fs-4).`,
    `- Two to five full-bleed moments in total, not one per section.`,
    `- One or two deliberate grid breaks in total.`,
    `- One image between 45 and 90vh. Never taller than the screen.`,
    `- The finished page is 4 to 9 screens at 1440x900.`,
    ``,
    `DENSITY IS THE POINT. Every section carries real content, not a heading and space: what each service covers, what it costs, who turns up, what happens at each step, the answer under each question. Most sections should hold 40 words or more. A visitor deciding whether to call is looking for what you do, what it costs and who arrives — a page that answers none of those, however beautifully set, sells nothing.`,
    ``,
