...
function buildCallScript(lead: Lead, playbook: Playbook): string {
  ...
  return [
    ...
    `Opener: "Hi, this is BarakahSoft — we just finished a free redesign of your homepage, want me to send it over while we're on the phone?"`,
...
export function buildClosePlanSteps(lead: Lead, playbook: Playbook): ClosePlanStepInput[] {
  ...
  return [
    { step_number: 1, kind: "call", scheduled_at: null, script_or_template: buildCallScript(lead, playbook) },
    {
      step_number: 2,
      kind: "email",
